...
def test_get_latest_snapshot_version():
    ...
    latest_jar = client.get_latest_version_url('com.smartertravel.metrics.aop.st-metrics', 'jar')
    print(latest_jar)
    ...
    latest_sources = client.get_latest_version_url('com.smartertravel.metrics.aop.st-metrics', 'jar', descriptor='sources')
...
def test_get_latest_snapshot_versions():
    ...
    latest_jars = client.get_latest_versions_urls('com.smartertravel.metrics.aop.st-metrics', 'jar')
    print(latest_jars)

    assert isinstance(latest_jars, list), "Expected list result from latest versions jar call"
    assert 1 <= len(latest_jars)

    latest_sources = client.get_latest_versions_urls('com.smartertravel.metrics.aop.st-metrics', 'jar', descriptor='sources')
    print(latest_sources)
